...
## 3. Commuter Transportation Mode Split (uncertain — commuters only)

Solo, carpool, transit, and other sum to **1.0**. Only solo and carpool enter the parking vehicle formula (§5).

### Data / assumption (adjusted for regional context)
...
solo_driver_rate ≈ **0.80** · carpool_rate ≈ **0.10** · transit_rate ≈ **0.07** · other_rate ≈ **0.03**
...
`BE/scripts/generate-campus-occupancy-from-scrape.mjs` builds **low / high** student vehicle counts per time slot from `totalEnrolledInClass`. **Both** scenarios use **commuter_share = 2,019 / 2,319** (§1). Endpoints for uncertain inputs follow **§2–§4**:

- **Low scenario (fewer cars):** attendance **75%**, commuter share **2,019 / 2,319**, solo driver **75%**, carpool **5%**, carpool size **2.5**
- **High scenario (more cars):** attendance **85%**, commuter share **2,019 / 2,319**, solo driver **85%**, carpool **15%**, carpool size **2.0**

Formula per §5: `commuters × solo_rate + commuters × carpool_rate / carpool_size` (transit/other → 0 vehicles).

**Instructors:** **0.72–0.93** vehicles per assumed instructor (range).  
**Non-teaching staff:** staff headcount range for the slot (StaffData.md §5) × **0.58–0.88** vehicles per person.
